...
describe('BandInput component', () => {
	...
	it('changes the local state on input change', () => {
		...
		expect(
			wrapper.state('name'),
			"BandInput should mount with this.state.name equal to ''"
		).to.equal('')
		let input = wrapper.find('input').first()
		input.simulate('change', { target: { value: 'Hello' } })
		expect(wrapper.state('name'), 'BandInput state did not contain the correct value').to.equal(
			'Hello'
		)
	})
	...
	it('calls dispatch when form is submitted', () => {
		...
		expect(wrapper.find('input').length > 0, 'No input elements found in the application').to.equal(
			true
		)
		let input = wrapper.find('input').first()
...
describe('Bands Container', () => {
	...
	it('renders each li with the correct name', () => {
		...
		expect(wrapper.name()).to.contain('The Black Keys')
		expect(wrapper.name()).to.contain('The White Stripes')
		expect(wrapper.name()).to.contain('Black Moth Super Rainbow')
	})
})
